...
def test_file_encrypt_decrypt_generate_same_data():
    with patch("encryption.file_cypher.load_data") as mock_load_data, \
        ...
        encrypted_file_name, encrypted_data = file_cypher.FileEnconding(key).encode_file(file_name)
        mock_load_data.return_value = encrypted_data
        decrypted_file_name, decrypted_data = file_cypher.FileEnconding(key).decode_file(encrypted_file_name)
